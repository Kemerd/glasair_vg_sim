...
| case | shape | pitch | count | Cd | Δ vs clean |
|---|---|---|---|---|---|
| clean_a02 | — | — | — | 0.01064 | — |
| **vg12d70b10_a02** | delta | 70 | pair | 0.01274 | **+19.8%** |
| vg12ssingle_a02 | stol | 50 | single | 0.01336 | +25.6% |
| vg12d50b10_a02 | delta | 50 | pair | 0.01344 | +26.3% |
| vg12single_a02 | rect | 50 | single | 0.01431 | +34.5% |
| vg12s50_a02 | stol | 50 | pair | 0.01682 | +58.1% |
| vg12p50_a02 | rect | 50 | pair | 0.01981 | +86.2% |
...
4. **Single-alternating halves the cruise tax** (+34% vs the pair's +86%) — the
   single biggest lever for the tradeoff. But a *flat* single loses lift at
   stall (−9%); a *swept* single keeps it (+1.9%). So **swept + single + wide**
   is the leading hypothesis for the cruise-friendly optimum, now being tested.

5. **Steadiness tracks reattachment quality.** vg12p70 and the single fins show
   the smallest limit-cycle pk-pk (0.013–0.017) — the flow is most solidly
   reattached, not just lower on average.
...
| config | 25% span | 40% span | 55% span | VGs @ 25% span |
|---|---|---|---|---|
| **delta β10, 70 mm** | **~1.5 kt** | **~2.4 kt** | **~3.3 kt** | 25 |
| delta β10, 50 mm | ~2.0 kt | ~3.2 kt | ~4.3 kt | 36 |
| orig rect-pair β15, 50 mm | ~6.3 kt | ~9.7 kt | ~13 kt | 36 |

**The 70 mm β10 delta drops the cruise penalty to ~1.5–3 kt** — a third of the
original config's cost — while needing ~30% fewer VGs. Caveat: the slice is
